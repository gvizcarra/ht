...
while ($row = mysql_fetch_assoc($res))
{
	...
	//echo "<br>Zona: ".$zonas[$idZ];
		...
		echo "<div class=\"titulo\"> Balance de Energia Año Movil</div>";
...
$sqlEnergia = "SELECT SUM(e.cantidad) AS cantidadEnergia,
				e.idzona,e.tipo,e.externoInterno AS extInt
 				FROM energia AS e 
					...
					WHERE  CONCAT(e.anio,'-',e.mes,'-01')  
							BETWEEN (DATE_ADD('".$idAnio."-".$idMes."-01', INTERVAL - 11  MONTH)) AND CONCAT('".$idAnio."','-','".$idMes."','-01')
						AND e.idZona IN (
												SELECT be.idzona 
												FROM energia  be
												WHERE be.anio = '".$idAnio."' AND be.mes = '".$idMes."'
												AND (
														SELECT COUNT(distinct(tipo)) AS total FROM  energia 
														WHERE anio = '".$idAnio."' AND mes = '".$idMes."' and idZona= be.idZona
												) >1
												GROUP BY be.idZona
												HAVING SUM(be.cantidad)>0
										 )
   			   GROUP BY e.idZona , e.externoInterno, e.tipo;";		
			 -//  anio = '".$idAnio."' AND mes = '".$idMes."'
$resEnergia = mysql_query($sqlEnergia);
$qtyRows = mysql_num_rows($resEnergia);
