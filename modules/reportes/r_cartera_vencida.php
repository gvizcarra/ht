...
if($MesesSelect =="")
		...
		FROM v_estrategias where id_mes <='".$MesesSelect."' and tipo_estrategia='2'
		group by id_zona,clave_estrategia,id_tipo";						
		echo "<div class=\"titulo\"> Cartera vencida </div>";
		echo "<div class=\"subtitulo\"> Avance de Programa 2015 </div>";
...
while ($row = mysql_fetch_assoc($res))
{
	...
	if($row["id_tipo"]=='1')
	 ...
	 //Total Acumulado $eap 
	 $eap[$row["id_zona"]] =$e1p[$row["id_zona"]]+$e2p[$row["id_zona"]]+$e3p[$row["id_zona"]]+$e4p[$row["id_zona"]];
	 $ear[$row["id_zona"]] =$e1r[$row["id_zona"]]+$e2r[$row["id_zona"]]+$e3r[$row["id_zona"]]+$e4r[$row["id_zona"]];
	 
	 //$eap[$row["id_zona"]] += $row["programado_estrategia"];
	// $ear[$row["id_zona"]] += $row["real_estrategia"]
	 	 
	 //Total Divisional x estrategia
	 $edtp = $ed1p+$ed2p+$ed3p+$ed4p;
	 $edtr = $ed1r+$ed2r+$ed3r+$ed4r;
  ...
  <head>
    ...
    <link href="../../css/style.css" rel="stylesheet" type="text/css">
  	<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
        ...
        self.location='r_cartera_vencida.php?cat=' + val ;
        }
...
<table border="1" id="datos" class="bordered" >
 <thead><tr>
    ...
    <th align="center">&nbsp;Día del Corte Masivo  con ASEMED.</th>
    <th align="center">&nbsp;Gestión de Cobranza y Recuperacion de Incobrables.</th>
    <th align="center">&nbsp;Convenios y Gestiones con Gobiernos para la Recuperación de Adeudos Pendientes.</th>
    <th align="center">&nbsp;Cumplimiento del Calendario de Cortes y Efectividad con ASEMED. </th>
